import {
  ...
  COLON,
  SEMICOLON,
} from "../data/regexSet.ts";
import { getFullWidthCharsMapping } from "../data/fullwidthCharsMapping.ts";
import { BishengMainConfig, BishengMainFeature } from "../types/index.ts";
import {
  group,
  set,
  not,
...
export const biShengFormat = (content: string, config?: BishengMainConfig): string => {
  ...
  type data_key = keyof typeof replaceSchema;

  Object.keys(replaceSchema).forEach((schema: data_key) => {
